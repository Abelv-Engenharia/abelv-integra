...
const TreinamentosNormativo = () => {
  ...
  const [treinamentosNormativos, setTreinamentosNormativos] = useState<{
    id: string;
    nome: string;
  ...
  function buildCertificadoFileName(nomeTreinamento: string, matriculaFuncionario: string, nomeFuncionario: string, ext: string): string {
  ...
  async function uploadCertificadoPersonalizado(file: File): Promise<string | null> {
    ...
    const {
      data,
      error
    } = await supabase.storage.from(BUCKET_CERTIFICADOS).upload(fileName, file, {
      upsert: true,
    ...
    } = await supabase.storage.from(BUCKET_CERTIFICADOS).createSignedUrl(fileName, 60 * 60 * 24 * 7); // 7 dias
  ...
  const onSubmit = async (data: FormValues) => {
    ...
    try {
      ...
      const result = await criarTreinamentoNormativo({
        ...
        // FIX: Passar o objeto Date diretamente
        ...
        <CardContent>
          {isLoading ? <div className="flex justify-center items-center py-8">
              ...
              <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
                <div className="mb-4">
                  <FormField control={form.control} name="ccaId" render={({
                field
              }) => <FormItem>
                    ...
                    }).map(cca => <SelectItem key={cca.id} value={String(cca.id)}>
                                  {cca.codigo} - {cca.nome}
                                </SelectItem>)}
                          </SelectContent>
                        </Select>
                ...
                field
                        ...
                        <Select onValueChange={field.onChange} value={field.value}>
                          ...
                          <SelectContent>
                            ...
                            {filteredFuncionarios.map(funcionario => <SelectItem key={funcionario.id} value={funcionario.id}>
                                {funcionario.nome}
                              </SelectItem>)}
                          </SelectContent>
                        </Select>
                ...
                <FormField control={form.control} name="treinamentoId" render={({
              field
            }) => <FormItem>
                      ...
                      <Select onValueChange={field.onChange} defaultValue={field.value}>
                        ...
                        <SelectContent>
                          {treinamentosNormativos.map(treinamento => <SelectItem key={treinamento.id} value={treinamento.id}>
                              {treinamento.nome}
                            </SelectItem>)}
                        </SelectContent>
                      </Select>
              ...
              field
                ...
                if (e.target.files && e.target.files[0]) {
                  ...
                  {certificadoFile && form.getValues("treinamentoId") && selectedFuncionario && <div className="text-xs text-blue-600 mt-1 font-mono">
                      Nome no bucket:{" "}
                      {buildCertificadoFileName(getTreinamentoNomeById(form.getValues("treinamentoId")) || "", selectedFuncionario.matricula, selectedFuncionario.nome, "pdf")}
                    </div>}
                  {certificadoFile && <div className="text-xs text-green-600 mt-1">
