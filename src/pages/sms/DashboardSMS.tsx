...
function StatCard({
  ...
  return <article className="rounded-lg border border-border bg-card text-card-foreground p-4 shadow-sm">
      <div className="flex items-center gap-3">
        {Icon && <div className="inline-flex h-8 w-8 items-center justify-center rounded-md bg-muted">
...
export default function DashboardSMS() {
  const {
    counts,
    loading: loadingPiramide
  } = usePiramideOcorrencias();
  const {
    data: userCCAs = []
  } = useUserCCAs();
  ...
  const idsmsMedia = idsmsData.length > 0 ? idsmsData.reduce((sum, item) => sum + item.idsms_total, 0) / idsmsData.length : 0;
  ...
  useEffect(() => {
    ...
    const desc = "Dashboard SMS com indicadores proativos e reativos do sistema de SMS.";
  ...
  return <div className="px-4 py-6 sm:px-6 lg:px-8">
      ...
      <main className="space-y-8">
        {/* Filtros */}
        <SMSDashboardFilters year={year} month={month} ccaId={ccaId} setYear={setYear} setMonth={setMonth} setCcaId={setCcaId} />

        {/* Índice de Desenvolvimento de SMS */}
        <section aria-labelledby="idsms" className="space-y-4">
          <div className="flex items-center justify-between bg-sky-200">
            <h2 id="idsms" className="text-lg font-semibold">
              Índice de Desenvolvimento de SMS
            </h2>
          </div>
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            <StatCard title="IDSMS Médio" value={loadingIDSMS ? "..." : `${idsmsMedia.toFixed(1)}`} subtitle="média geral" icon={Gauge} />
            <StatCard title="Status Geral" value={loadingIDSMS ? "..." : statusGeral} subtitle="baseado no IDSMS médio" icon={CheckCircle2} />
          </div>
        </section>
        ...
        <section aria-labelledby="indicadores-proativos" className="space-y-4">
          ...
          <article className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <StatCard title="Total de Desvios" value={loading ? "..." : desviosStats?.totalDesvios || 0} subtitle="Todos os desvios registrados" icon={AlertTriangle} />
              <StatCard title="Ações Concluídas" value={loading ? "..." : `${desviosStats?.acoesCompletas || 0} (${desviosStats?.percentualCompletas || 0}%)`} subtitle="Ações finalizadas" icon={CheckCircle2} />
              <StatCard title="Ações em Andamento" value={loading ? "..." : `${desviosStats?.acoesAndamento || 0} (${desviosStats?.percentualAndamento || 0}%)`} subtitle="Ações sendo executadas" icon={Clock} />
              <StatCard title="Ações Pendentes" value={loading ? "..." : `${desviosStats?.acoesPendentes || 0} (${desviosStats?.percentualPendentes || 0}%)`} subtitle="Ações ainda não iniciadas" icon={AlertTriangle} />
            </div>
          </article>
          ...
          <article className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <StatCard title="Percentual investido" value={loading ? "..." : `${treinamentoData?.percentualHorasInvestidas?.toFixed(2) || 0}%`} subtitle="Do total de horas trabalhadas (HHT)" icon={TrendingUp} />
              <StatCard title="Horas Trabalhadas (HHT)" value={loading ? "..." : treinamentoData?.totalHHT?.toLocaleString() || 0} subtitle="no período selecionado" icon={Clock} />
              <StatCard title="Meta de Horas (2,5% HHT)" value={loading ? "..." : Math.round(treinamentoData?.metaHoras || 0).toLocaleString()} subtitle="Meta no período" icon={Gauge} />
              <StatCard title="Horas Totais de Treinamentos" value={loading ? "..." : Math.round(treinamentoData?.totalHorasTreinamento || 0).toLocaleString()} subtitle="Realizadas no período" icon={CheckCircle2} />
            </div>
          </article>
          ...
          <article className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-6">
              <StatCard title="Adesão HSA (real)" value={loading ? "..." : `${hsaDetailedStats?.aderenciaReal?.toFixed(2) || 0}%`} subtitle="Realizadas vs Programadas" icon={TrendingUp} />
              <StatCard title="Adesão HSA (ajustada)" value={loading ? "..." : `${hsaPercentage?.toFixed(2) || 0}%`} subtitle="Incluindo não programadas" icon={TrendingUp} />
              <StatCard title="Inspeções Programadas" value={loading ? "..." : hsaDetailedStats?.inspecoesProgramadas || 0} subtitle="A realizar + Replanejadas + Não realizadas" icon={Clock} />
              <StatCard title="Inspeções Realizadas" value={loading ? "..." : hsaDetailedStats?.inspecoesRealizadas || 0} subtitle="Inspeções concluídas" icon={CheckCircle2} />
              <StatCard title="Não Programadas" value={loading ? "..." : hsaDetailedStats?.naoProgramadas || 0} subtitle="Inspeções não programadas" icon={BarChart3} />
              <StatCard title="Não Realizadas" value={loading ? "..." : hsaDetailedStats?.naoRealizadas || 0} subtitle="Inspeções não executadas" icon={AlertTriangle} />
            </div>
          </article>
        </section>
        ...
        <section aria-labelledby="indicadores-reativos" className="space-y-4">
          ...
          <article className="space-y-3">
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-4">
              <StatCard title="Ocorrências com perda de dias" value={loading ? "..." : ocorrenciasStats?.ocorrenciasComPerdaDias || 0} icon={AlertTriangle} subtitle="AC CPD" />
              <StatCard title="Ocorrências sem perda de dias" value={loading ? "..." : ocorrenciasStats?.ocorrenciasSemPerdaDias || 0} icon={CheckCircle2} subtitle="AC SPD" />
              <StatCard title="Incidentes" value={loading ? "..." : ocorrenciasStats?.incidentes || 0} icon={BarChart3} subtitle="INC DM, INC SDM, INC AMB" />
              <StatCard title="Desvios de Alto Potencial" value={loading ? "..." : ocorrenciasStats?.desviosAltoPotencial || 0} icon={AlertTriangle} subtitle="DAP" />
              <StatCard title="Dias Perdidos" value={loading ? "..." : ocorrenciasStats?.totalDiasPerdidos || 0} icon={Clock} subtitle="Total de dias perdidos" />
              <StatCard title="Dias Debitados" value={loading ? "..." : ocorrenciasStats?.totalDiasDebitados || 0} icon={Clock} subtitle="Total de dias debitados" />
              <StatCard title="Ocorrências Concluídas" value={loading ? "..." : ocorrenciasStats?.ocorrenciasConcluidas || 0} icon={CheckCircle2} subtitle="Status concluído/fechado" />
              <StatCard title="Ocorrências Pendentes" value={loading ? "..." : ocorrenciasStats?.ocorrenciasPendentes || 0} icon={AlertTriangle} subtitle="Status aberto/pendente" />
            </div>
          </article>
