...
const ComunicadoModal: React.FC<ComunicadoModalProps> = ({
  ...
  const getFileUrl = (url: string) => {
    if (url.includes('comunicados-anexos')) {
      ...
      const {
        data
      } = supabase.storage.from('comunicados-anexos').getPublicUrl(fileName || '');
  ...
  const renderAnexo = () => {
    ...
    if (isImage(fileName)) {
      return <div className="mt-4">
          ...
          <img src={fileUrl} alt={fileName} className="max-w-full h-auto rounded-lg border shadow-sm" style={{
          maxHeight: '800px'
        }} />
        </div>;
    }
    if (isPdf(fileName)) {
      return <div className="mt-4">
          ...
          <iframe src={fileUrl} className="w-full h-[700px] border rounded-lg shadow-sm" title={fileName} />
          <div className="mt-2">
  ...
  return <Dialog open={open} onOpenChange={() => {}}>
      <DialogContent className="max-w-7xl w-[95vw] max-h-[90vh] my-0 mx-[17px] py-[39px] px-[44px]">
        <DialogHeader>
        ...
        <ScrollArea className="max-h-[80vh] pr-4">
          <div className="space-y-8 p-4">
            {comunicado.descricao && <div className="prose max-w-none">
        ...
        <div className="flex justify-end pt-4 border-t">
          <Button onClick={handleCiencia} disabled={loading} className="min-w-[100px]">
