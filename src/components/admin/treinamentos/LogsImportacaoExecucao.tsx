...
interface LogImportacaoExecucao {
  id: string;
...
const fetchLogsImportacaoExecucao = async (): Promise<LogImportacaoExecucao[]> => {
  const { data, error } = await supabase
    .from('logs_importacao_execucao_treinamentos')
    .select(`
      *,
      profiles!logs_importacao_execucao_treinamentos_usuario_id_fkey (
        nome,
        email
      )
    `)
  ...
  if (error) {
    console.error('Erro ao buscar logs de importação de execução:', error);
    throw error;
  }
...
export const LogsImportacaoExecucao = () => {
  const { data: logs = [], isLoading, error } = useQuery({
    queryKey: ['logs-importacao-execucao-treinamentos'],
    queryFn: fetchLogsImportacaoExecucao,
  });
