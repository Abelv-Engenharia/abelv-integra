...
type ChartItem = {
  ...
  fullName?: string;  // nome completo p/ tooltip (opcional)
  value: number;      // total de desvios na disciplina
...
const DesviosByDisciplineChart = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const chartData = await fetchDesviosByDiscipline(normalizedFilters);
  ...
  // Trunca ticks longos no eixo X para não colidir
               ...
               <BarChart 
                ...
                onClick={async (data) => {
                  if (data && data.activePayload?.[0]?.payload?.name) {
                    await navigateToConsulta({ disciplina: data.activePayload[0].payload.name });
                  }
                }}
