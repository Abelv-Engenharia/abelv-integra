...
export const EditarFaturaModal = ({ fatura, open, onClose, onSave }: EditarFaturaModalProps) => {
  ...
  const form = useForm<FaturaFormData>({
    ...
    defaultValues: {
      dataemissaofat: fatura.dataemissaofat,
      agencia: fatura.agencia,
      numerodefat: fatura.numerodefat,
      protocolo: fatura.protocolo,
      datadacompra: fatura.datadacompra,
      viajante: fatura.viajante,
      tipo: fatura.tipo,
      hospedagem: fatura.hospedagem || "",
      origem: fatura.origem,
      destino: fatura.destino,
      checkin: fatura.checkin || "",
      checkout: fatura.checkout || "",
      comprador: fatura.comprador,
      valorpago: fatura.valorpago.toString(),
      motivoevento: fatura.motivoevento,
      cca: fatura.cca,
      centrodecusto: fatura.centrodecusto,
      antecedencia: fatura.antecedencia?.toString() || "",
      ciaida: fatura.ciaida || "",
      ciavolta: fatura.ciavolta || "",
      possuibagagem: fatura.possuibagagem,
      valorpagodebagagem: fatura.valorpagodebagagem?.toString() || "",
      observacao: fatura.observacao || "",
      quemsolicitouforapolitica: fatura.quemsolicitouforapolitica || "",
      dentrodapolitica: fatura.dentrodapolitica,
      codconta: fatura.codconta || "",
      contafinanceira: fatura.contafinanceira || "",
    },
  });
  ...
  const onSubmit = (data: FaturaFormData) => {
    ...
    const updatedFatura: FaturaIntegra = {
      ...fatura,
    ...
    const updatedFaturas = faturas.map((f) =>
      f.id === fatura.id ? updatedFatura : f
    );

    localStorage.setItem("faturas_integra", JSON.stringify(updatedFaturas));
      ...
      <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-6">
            ...
            {/* Restante do formulário igual ao NovaFaturaModal */}
            {/* Dados da Viagem */}
            <div className="space-y-4">
