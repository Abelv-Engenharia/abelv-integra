...
import { Users, Plus, Save, Download, Trash2, Edit, AlertCircle, FileText } from 'lucide-react';
...
export default function EfetivoAbelvPJ() {
  const { toast } = useToast();
  const { colaboradores, adicionarColaborador, atualizarColaborador, removerColaborador } = useColaboradoresAbelvPJ();
  ...
  const colaboradoresFiltrados = colaboradores.filter(col => {
    const matchCca = !ccaSelecionado || true; // CCA filter would be applied here
    ...
    try {
      if (editandoId) {
        ...
        toast({
          ...
          description: "Colaborador atualizado no efetivo abelv + pj"
        });
      } else {
        await adicionarColaborador(novoColaborador);
        toast({
          title: "Adicionado com sucesso",
          description: "Colaborador incluído no efetivo abelv + pj. Ele aparecerá automaticamente no Controle Diário."
        });
      }
  ...
  const handleExcluir = (id: string) => {
    ...
    toast({
      title: "Removido",
      description: "Colaborador removido do efetivo abelv + pj"
    });
  };
  ...
  const handleExportar = () => {
    toast({
      title: "Exportando",
      description: "Relatório de efetivo abelv + pj sendo gerado..."
    });
  };
    ...
    <div className="min-h-screen p-8 animate-in fade-in-0 duration-500">
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Efetivo obra</h1>
            <p className="text-muted-foreground">Cadastro e atualização de colaboradores abelv + pj</p>
          </div>
        </div>
        ...
        <Alert>
          ...
          <AlertDescription>
            Os colaboradores cadastrados aqui aparecem automaticamente no <strong>Controle Diário - Abelv + PJ</strong> para registro de status diário.
          </AlertDescription>
        </Alert>
                      ...
                      <TableRow key={colaborador.id}>
                        ...
                        <TableCell>
                          <div className="flex items-center gap-2">
                            {colaborador.nome}
                            {(colaborador as any).validacao_admissao_id && (
