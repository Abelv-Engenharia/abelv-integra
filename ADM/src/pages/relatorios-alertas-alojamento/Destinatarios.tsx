...
export default function Destinatarios() {
  ...
  const handleAdicionarDestinatario = () => {
    ...
    if (novoDestinatario.modulos.length === 0) {
    ...
    const novosDestinatarios = emailList.map((email, index) => ({
      ...
      modulos: novoDestinatario.modulos
    }));

    setDestinatarios([...destinatarios, ...novosDestinatarios]);
    setNovoDestinatario({ obra: '', responsavel: '', email: '', cargo: '', modulos: [] });
    ...
    <div className="min-h-screen p-8 animate-in fade-in-0 duration-500">
      <div className="space-y-6">
        ...
        <div className="space-y-4">
          <div>
            <h1 className="text-3xl font-bold tracking-tight">Configuração de Destinatários</h1>
            <p className="text-muted-foreground">Cadastro de responsáveis por obra para recebimento de alertas</p>
          </div>
        </div>
        ...
        <Card>
          <CardHeader>
            ...
            <CardDescription>
              Lista de destinatários para alertas gerais do sistema
            </CardDescription>
          </CardHeader>
        ...
        <Card>
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              <Mail className="h-5 w-5" />
              Configuração de Destinatários Administrativo por CCA
            </CardTitle>
            <CardDescription>
              Defina os responsáveis administrativos por obra para validação dos módulos de Contratos de Alojamento e Gestão de Alojamentos - Mensal
            </CardDescription>
          </CardHeader>
          <CardContent className="space-y-4">
            <div className="flex justify-end">
              <Dialog open={openDestinatarioDialog} onOpenChange={setOpenDestinatarioDialog}>
                ...
                <DialogContent className="max-w-md">
                  <DialogHeader>
                    ...
                    <DialogDescription>
                      Cadastre um novo responsável por obra para recebimento de alertas
                    </DialogDescription>
                  </DialogHeader>
                  <div className="space-y-4 py-4">
                    <div className="space-y-2">
                    ...
                    <div className="space-y-3">
                      <Label className="text-red-500">Módulos de Responsabilidade*</Label>
                      <div className="space-y-2">
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="modulo-contratos"
                            checked={novoDestinatario.modulos.includes('contratos')}
                            onCheckedChange={(checked) => {
                              if (checked) {
                                setNovoDestinatario({ 
                                  ...novoDestinatario, 
                                  modulos: [...novoDestinatario.modulos, 'contratos'] 
                                });
                              } else {
                                setNovoDestinatario({ 
                                  ...novoDestinatario, 
                                  modulos: novoDestinatario.modulos.filter(m => m !== 'contratos') 
                                });
                              }
                            }}
                          />
                          <label
                            htmlFor="modulo-contratos"
                            className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                          >
                            Contratos de Alojamento
                          </label>
                        </div>
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            id="modulo-mensal"
                            checked={novoDestinatario.modulos.includes('mensal')}
                            onCheckedChange={(checked) => {
                              if (checked) {
                                setNovoDestinatario({ 
                                  ...novoDestinatario, 
                                  modulos: [...novoDestinatario.modulos, 'mensal'] 
                                });
                              } else {
                                setNovoDestinatario({ 
                                  ...novoDestinatario, 
                                  modulos: novoDestinatario.modulos.filter(m => m !== 'mensal') 
                                });
                              }
                            }}
                          />
                          <label
                            htmlFor="modulo-mensal"
                            className="text-sm font-medium leading-none peer-disabled:cursor-not-allowed peer-disabled:opacity-70"
                          >
                            Gestão de Alojamentos - Mensal
                          </label>
                        </div>
                      </div>
                    </div>
                    ...
                    <TableRow key={destinatario.id}>
                      ...
                      <TableCell>
                        <div className="flex gap-1 flex-wrap">
                          {destinatario.modulos.includes('contratos') && (
                            <Badge variant="secondary">Contratos</Badge>
                          )}
                          {destinatario.modulos.includes('mensal') && (
