...
interface NovoSemPararModalProps {
  open: boolean
  onOpenChange: (open: boolean) => void
}

export function NovoSemPararModal({ open, onOpenChange }: NovoSemPararModalProps) {
  ...
  const onSubmit = async (values: FormValues) => {
    ...
    try {
      ...
      const existingSemParar = JSON.parse(localStorage.getItem("semparar") || "[]")
      const semParar = [...existingSemParar, novoSemParar]
      localStorage.setItem("semparar", JSON.stringify(semParar))

      toast({
        title: "Sucesso",
        description: "Registro Sem Parar cadastrado com sucesso!",
      })
      ...
      toast({
        title: "Erro",
        description: "Erro ao cadastrar registro. Tente novamente.",
        variant: "destructive",
      })
    } finally {
      setLoading(false)
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Novo Sem Parar</DialogTitle>
        </DialogHeader>
