...
export default function EletricaListaRegistros() {
  ...
  const carregarRegistros = async () => {
    try {
      ...
      const atividadesPorRelatorio: Record<string, any[]> = {}
      ...
      const registrosProcessados = (relatoriosData || []).map(registro => {
        ...
        return {
          ...
          cca: registro.ccas,
          encarregado: registro.encarregados_eletrica,
  ...
  const excluirRegistro = async (id: string) => {
    try {
      ...
      // Para cada atividade principal, excluir os registros relacionados
      for (const atividade of atividadesPrincipais || []) {
        // Excluir registro_atividades_eletrica
        await supabase
  ...
  const visualizarRelatorio = async (id: string) => {
    try {
      ...
      const relatorioCompleto = {
        ...relatorio,
        atividades_eletricas: atividades
      }
      ...
      <div className="max-w-7xl mx-auto space-y-6">
        ...
        <Card className="border-l-4 border-l-blue-500">
          <CardHeader className="bg-blue-600 text-white">
            <CardTitle className="text-xl font-bold">
              Lista de Registros - Elétrica/Instrumentação
            </CardTitle>
          </CardHeader>
        </Card>
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
            <Card>
              <CardContent className="p-4">
                <div className="text-2xl font-bold text-blue-600">{registrosFiltrados.length}</div>
                  ...
                  {registrosFiltrados.map((registro) => (
                    <TableRow key={registro.id}>
                      ...
                      <TableCell className="text-center">
                        <Badge variant="secondary" className="flex items-center justify-center space-x-1 bg-blue-100 text-blue-700 border-blue-300">
                          <span>{registro.atividades_count}</span>
                        </Badge>
                      </TableCell>
                      <TableCell className="text-center">
                        <Badge variant="outline" className="flex items-center justify-center space-x-1 text-blue-600 border-blue-300">
        ...
        <Dialog open={showPreview} onOpenChange={setShowPreview}>
          <DialogContent className="max-w-5xl max-h-[90vh]">
            ...
            <RelatorioPreviewEletrica relatorio={relatorioCompleto} />
          </DialogContent>
        </Dialog>
      </div>
    </Layout>
