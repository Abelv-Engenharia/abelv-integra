-- Atualizar a função processar_configuracoes_emails para incluir relatórios automáticos
...
BEGIN
  ...
  FOR config_record IN 
    ...
    WHERE ativo = true 
    AND hora_envio = date_trunc('hour', now()::time)
  LOOP
    should_send := false;
    ...
    IF should_send THEN
      ...
      IF config_record.tipo_relatorio IS NOT NULL THEN
        ...
        corpo_final := corpo_final || '<p>Relatório de ' || config_record.tipo_relatorio || ' dos últimos ' || COALESCE(config_record.periodo_dias, 30) || ' dias será anexado automaticamente.</p>';
      END IF;
