-- Re-run without IF NOT EXISTS for CREATE POLICY (not supported)
...
drop policy if exists "Admins can manage funcionario_supervisores" on public.funcionario_supervisores;
drop policy if exists "Supervisors can view their mappings" on public.funcionario_supervisores;

-- 2) RLS for mapping table: admins can manage; supervisors can read their own mappings
create policy "Admins can manage funcionario_supervisores"
...
create policy "Supervisors can view their mappings"
...
-- Ensure admin ALL policy exists (already present); add supervisor view policy
